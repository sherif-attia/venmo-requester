...
def test_it_should_handle_sheet_errors(mock_build, mock_credentials):
    ...
    mock_service = Mock()
    mock_service.spreadsheets.return_value.values.return_value.get.return_value.execute.side_effect = Exception("API Error")
    mock_build.return_value = mock_service
...
def test_it_should_handle_invalid_amount_format(mock_build, mock_credentials):
    ...
    mock_service.spreadsheets.return_value.values.return_value.get.return_value.execute.return_value = {
        'values': [
            ['venmo_id', 'note', 'amount'],
            ['user1', 'March Rent', 'not-a-number']
        ]
    }
    mock_build.return_value = mock_service
