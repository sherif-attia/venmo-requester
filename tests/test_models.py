...
def test_it_should_handle_payment_success_and_failure_states():
    ...
    success_result = PaymentResult(request=request, success=True, error_message=None)
    assert success_result.success is True
    assert success_result.error_message is None

    # Test failed payment with error message
    error_message = "Insufficient funds"
    failed_result = PaymentResult(request=request, success=False, error_message=error_message)
...
def test_it_should_properly_mask_sensitive_config_values():
    ...
    config = VenmoConfig(
        access_token="secret_token", client_id="secret_id", client_secret="secret_secret"
    )
